...
def get_engine(onnx_file_path, engine_file_path="", precision ='fp32', TORCH_QUANTIZATION =False , gen_force = False):
            ...
            if precision == "fp16":
                ...
                if not builder.platform_has_fast_int8:
                    ...
                    if TORCH_QUANTIZATION:
                        ...
                        config.int8_calibrator = EngineCalibrator(TRT_LOGGER, calib_cache)
                        if not os.path.exists(calib_cache):
                            calib_shape = [batch_size] + list(inputs[0].shape[1:])
                            calib_dtype = trt.nptype(inputs[0].dtype)
                            config.int8_calibrator.set_calibrator(batch_size, calib_shape, calib_dtype, './calib_data')
...
def main():
    ...
    t_outputs = []
    with get_engine(onnx_model_path, engine_file_path, precision, TORCH_QUANTIZATION,gen_force) as engine, engine.create_execution_context() as context:
