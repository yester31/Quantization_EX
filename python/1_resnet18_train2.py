...
def main():
    ...
    model = train_model(model_filename=model_filename, model=model, train_loader=train_loader, test_loader=test_loader, device=device, learning_rate=1e-2, num_epochs=num_epochs)

    save_model(model, f'{model_filename}_e{str(num_epochs)}.pth')
    model = load_model(model, f'{model_filename}_best.pth', device)
...
def model_onnx_export():
    ...
    export_model_path = f'model/{model_filename}_best.onnx'
    ...
    torch.onnx.export(model,  # pytorch model
                      ...
                      opset_version=16,  # the version of the opset
